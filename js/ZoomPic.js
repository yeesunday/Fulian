...
{   var old = this.aSort.pop();
	var old_first = this.aSort[0];
	var pre = (parseInt($(old_first).find('i').text())-1+20)%20;
	if(pre==0){pre=20;}
	$(old).find('i').text(pre);
	this.aSort.unshift(old);
	his.setUp()
},doNext:function()
{
	var old = this.aSort.shift();
	var old_last = this.aSort[this.aSort.length-1];
	var next  = (parseInt($(old_last).find('i').text())+1)%20;
	if(next==0){next=20;}
	$(old).find('i').text(next);
	this.aSort.push(old);
	this.setUp()
},doImgClick: function() {
    var _this = this;
        ...
        this.aSort[i].onclick = function() {
            if (this.index > _this.iCenter) {
                for (var i = 0; i < this.index - _this.iCenter; i++) {
                    var old = _this.aSort.shift();
                    var old_last = _this.aSort[_this.aSort.length - 1];
                    var next = (parseInt($(old_last).find('i').text()) + 1) % 20;
                    if (next == 0) {
                        next = 20;
                    }
                    $(old).find('i').text(next);
                    _this.aSort.push(old);
                }
                _this.setUp();
            } else if (this.index < _this.iCenter) {
                for (var i = 0; i < _this.iCenter - this.index; i++) {
                    var old = _this.aSort.pop();
                    var old_first = _this.aSort[0];
                    var pre = (parseInt($(old_first).find('i').text()) - 1 + 20) % 20;
                    if (pre == 0) {
                        pre = 20;
                    }
                    $(old).find('i').text(pre);
                    _this.aSort.unshift(old);
                }
                _this.setUp();
            }
        }
